...
def prepare_features(df):
    ...
    baseline_features = [
        ...
        # v2: 시장 상황 지표 추가
        'market_entry_ma_return_5d',    # 시장 단기 수익률
        'market_entry_ma_return_20d',   # 시장 중기 수익률
        'market_entry_volatility_20d',  # 시장 변동성
    ]
    
    # 사용 가능한 특성만 선택
    available_features = [f for f in baseline_features if f in df.columns]
    X = df[available_features].copy()
    y = df['label']  # v2: 시장 초과 수익률 기반 라벨
...
def evaluate_trade_quality_model(model, X, y, dataset_name):
    ...
    # 거래 품질별 분석
    print(f"\n💰 거래 품질 분석 (시장 초과 성과 기준):")
    true_positive_rate = cm[1,1] / (cm[1,0] + cm[1,1])
    false_positive_rate = cm[0,1] / (cm[0,0] + cm[0,1])
    print(f"  시장 초과 거래 정확히 식별: {true_positive_rate:.1%}")
    print(f"  시장 미달 거래를 초과로 잘못 분류: {false_positive_rate:.1%}")
    
    return metrics, y_prob

def train_trade_quality_baseline():
    """개별 거래 품질 평가 Baseline 모델 학습 - v2 (시장 초과 수익률 기반)"""
    
    print("🎯 개별 거래 품질 평가 모델 학습 v2 (시장 초과 수익률 기반)")
    print("==" * 25)
    
    # 1. 데이터 로드
    print("\n📂 데이터 로딩 중...")
    train_df = pd.read_csv('../data/baseline_train_v2.csv')
    val_df = pd.read_csv('../data/baseline_val_v2.csv')
    test_df = pd.read_csv('../data/baseline_test_v2.csv')
    ...
    joblib.dump(best_model, '../models/baseline_model_v2.pkl')
    joblib.dump(scaler, '../models/baseline_scaler_v2.pkl')
    ...
    metadata = {
        ...
        'label_type': 'excess_return > 0',  # v2: 시장 초과 수익률
        'training_date': pd.Timestamp.now().strftime('%Y-%m-%d')
    }
    
    with open('../models/baseline_metadata_v2.json', 'w') as f:
        import json
        json.dump(metadata, f, indent=2)
    
    print(f"\n✅ 모델이 ../models/ 폴더에 저장되었습니다")
    print(f"✅ 이 baseline v2는 시장 초과 수익률로 거래 품질을 평가합니다")
    print(f"✅ 종목/산업 편향 없음 - 순수 기술적 분석 + 시장 상황")
    
    return results, best_model

def analyze_trade_quality_patterns(model_path='../models/baseline_model_v2.pkl'):
    """거래 품질 패턴 심층 분석 - v2"""
    ...
    scaler = joblib.load('../models/baseline_scaler_v2.pkl')
    test_df = pd.read_csv('../data/baseline_test_v2.csv')
    ...
    # 2. 실제 초과 수익률과의 관계
    print("\n💰 품질 점수와 시장 초과 수익률 관계:")
    correlation = test_df['quality_score'].corr(test_df['excess_return'])
    ...
    for score_range, label in [(0.8, '최상급'), (0.6, '우수'), (0.4, '보통'), (0.2, '미흡')]:
        ...
        if mask.sum() > 0:
            ...
            print(f"    - 평균 초과 수익률: {subset['excess_return'].mean():.2%}")
            print(f"    - 시장 초과 비율: {subset['label'].mean():.1%}")

if __name__ == "__main__":
    # 1. 개별 거래 품질 평가 모델 학습 (v2)
