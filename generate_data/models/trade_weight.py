...
class TradeQualityEvaluator:
    ...
    def __init__(self, train_months=36, val_months=6, test_months=6, step_months=3):
        ...
        # 최적화된 가중치 저장 (entry, exit_timing, result)
        self.best_weights = {'entry': 0.30, 'exit_timing': 0.30, 'result': 0.40}  # 기본값

    def optimize_weights(self, df_train, verbose=True):
        """1단계: Trade Quality 가중치 최적화 (0.1~0.8 범위, 0.05 단위)"""
        if verbose:
            print("1단계: Trade Quality 가중치 최적화 시작")
        ...
        for entry in weights_range:
            for exit_timing in weights_range:
                result = 1.0 - entry - exit_timing
                # result도 0.1~0.8 범위 내에 있어야 함
                if 0.1 <= result <= 0.8:
        ...
        for i, weights in enumerate(weight_combinations):
            ...
            y = df_with_labels['trade_quality_score']
        ...
        if verbose:
            ...
            for i, result in enumerate(sorted_results[:5], 1):
                w = result['weights']
                print(f"  {i}. Entry:{w['entry']:.2f} Exit:{w['exit_timing']:.2f} Result:{w['result']:.2f} → R²={result['cv_r2']:.4f}")
    ...
    def _create_labels_with_weights(self, df, weights):
        ...
        entry_quality = df_with_buy['buy_signal_score']
        
        # 2. 청산 타이밍 품질 점수 계산
        exit_timing_quality = self._calculate_exit_timing_without_return(df)
        
        # 3. 최종 성과 점수 계산
        result_quality = self._score_return(df['return_pct'])
        
        # 가중치 적용
        df['trade_quality_score'] = (
            entry_quality * weights['entry'] +
            exit_timing_quality * weights['exit_timing'] +
            result_quality * weights['result']
        )
    ...
    def create_quality_score(self, df, weights=None, risk_scaler=None, eff_scaler=None, verbose=False):
        """ 거래 품질 점수 생성 """
        if verbose:
            print("거래 품질 점수 생성 중")
    ...
    def train_model_with_weight_optimization(self, df, hyperparameter_search=False, verbose=False):
        """2단계 최적화: 가중치 → 하이퍼파라미터"""
        if verbose:
            print("2단계 최적화 Trade Quality 모델 훈련 시작")
            print("=" * 50)
        ...
        if verbose:
            ...
            print(f"필수 데이터 필터링: {len(df_filtered):,}개 ({filter_ratio:.1f}%)")
        ...
        # 최적 가중치로 전체 데이터 라벨링
        df_with_score = self.create_quality_score(df_filtered, weights=self.best_weights, verbose=verbose)
    ...
    def predict_quality(self, df, verbose=False):
        ...
        if verbose:
            ...
            print(f"  평균 품질 점수: {predictions.mean():.1f}")

        return predictions

    def get_quality_interpretation(self, score):
        """거래 품질 점수 해석"""
        if score >= 85:
            return "매우 우수한 거래 품질"
        elif score >= 75:
    ...
    def _optimize_hyperparameters(self, X, y, verbose=False):
        ...
        if verbose:
            print(f"    총 {len(param_grid['max_depth']) * len(param_grid['learning_rate']) * len(param_grid['n_estimators']) * len(param_grid['subsample']) * len(param_grid['colsample_bytree']) * len(param_grid['reg_alpha']) * len(param_grid['reg_lambda'])}개 조합 테스트")
        ...
        if verbose:
            ...
            for i, (idx, row) in enumerate(top_5.iterrows(), 1):
                params_str = ", ".join([f"{k.replace('param_', '')}: {v}" for k, v in row.items() if k.startswith('param_')])
                print(f"    {i}. Score: {row['mean_test_score']:.4f} | {params_str}")
    ...
    def load_model(self, filename):
        ...
        self.best_weights = save_data.get('best_weights', {'entry': 0.30, 'exit_timing': 0.30, 'result': 0.40})
        self.is_trained = True
...
def main():
    ...
    print("="*70)
    print("  - 2단계 최적화: 가중치 최적화 + 하이퍼파라미터 최적화")
    print("  - 완료된 매수-매도 거래의 품질을 0-100점으로 평가")
    print("="*70)
    ...
    try:
        ...
        # 2단계 최적화 모델 학습
        print(f"\n🚀 2단계 최적화 모델 학습 시작...")
        result = evaluator.train_model_with_weight_optimization(train_df, hyperparameter_search=False, verbose=True)

        # 평가 함수 (최적화된 가중치 사용)
        def evaluate_model(evaluator, data, name):
            data_with_score = evaluator.create_quality_score(data, weights=evaluator.best_weights, verbose=False)
        ...
        print(f"\n Trade Quality 2단계 최적화 모델 학습 완료!")
        print(f" 저장된 모델: {model_filename}")
        print(f" 최종 사용된 가중치: {evaluator.best_weights}")
        ...
        if 'weight_results' in result:
            ...
            for i, w_result in enumerate(sorted_weights[:3], 1):
                w = w_result['weights']
                print(f"  {i}. Entry:{w['entry']:.2f} Exit:{w['exit_timing']:.2f} Result:{w['result']:.2f} → R²={w_result['cv_r2']:.4f}")
